/*=========================================================================
  ...
  Module:    $RCSfile: vtksmoothpolydatafilter2.cpp,v $
...
// The following code defines methods for the vtkSmoothPolyDataFilter2 class
//
...
vtkSmoothPolyDataFilter2::vtkSmoothPolyDataFilter2()
{
...
void vtkSmoothPolyDataFilter2::SetSource(vtkPolyData *source)
{
  this->SetInput(1, source);
}

vtkPolyData *vtkSmoothPolyDataFilter2::GetSource()
{
...
int vtkSmoothPolyDataFilter2::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  cout<<"Found\n\t" << numSimple << " simple vertices\n\t"
    ...
    for (i=0; i<numPts; i++) 
      {
      	cout<<"Verts["<<i<<"].type="<<vertex_type(Verts[i].type)<<endl;
  ...
  iterationNumber++ )
    ...
    if ( iterationNumber && !(iterationNumber % 5) )
      ...
      if (this->GetAbortExecute())
        {
	cout<<"ABOOOOOOOOOOOOOOOORT!!!!!!!!!!"<<endl;
        abortExecute = 1;
        break;
        }
      }
...
//       	cout<<"Verts["<<i<<"].type="<<(int)Verts[i].type<<endl;
  ...
  if ( this->GenerateErrorScalars )
	...
	cout<<"dist["<<i<<"]="<<sqrt(vtkMath::Distance2BetweenPoints(x1,x2))<<endl;
      }
    int idx = output->GetPointData()->AddArray(newScalars);
	cout<<"ERROR SCALARS STORED IN ARRAY "<<idx<<endl;
    output->GetPointData()->SetActiveAttribute(idx, vtkDataSetAttributes::SCALARS);
    for (i=0; i<numPts; i++)
    {
      double dist=newScalars->GetComponent(i-1,idx);
...
int vtkSmoothPolyDataFilter2::FillInputPortInformation(int port,
                                                      vtkInformation *info)
{
...
void vtkSmoothPolyDataFilter2::PrintSelf(ostream& os, vtkIndent indent)
{
