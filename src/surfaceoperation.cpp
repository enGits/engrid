...
vec3_t SurfaceOperation::transform(vtkIdType id_cnode, vtkIdType id_node)
{
  vec3_t x0, x;
  m_Grid->GetPoint(id_cnode, x0.data());
  m_Grid->GetPoint(id_node, x.data());
  if (m_StretchingFactor > 1) {
    vec3_t n0 = m_NodeNormal[id_cnode];
    vec3_t n = m_NodeNormal[id_node];
    vec3_t v = x - x0;
    double sf = max(1.0, 1.0 + (m_StretchingFactor - 1.0)*pow(n0*n, 5.0));
    //x = x0 + (1.0/sf)*v;
    v[2] /= m_StretchingFactor;
    x = x0 + v;
  }
  return x;
}

vec3_t SurfaceOperation::transform(vtkIdType id_cnode1, vtkIdType id_cnode2, vtkIdType id_cnode3, vtkIdType id_cnode4, vtkIdType id_node)
{
  vec3_t x1, x2,x3,x4, x;
  m_Grid->GetPoint(id_cnode1, x1.data());
  m_Grid->GetPoint(id_cnode2, x2.data());
  m_Grid->GetPoint(id_cnode3, x3.data());
  m_Grid->GetPoint(id_cnode4, x4.data());
  m_Grid->GetPoint(id_node, x.data());
  vec3_t x0 = 0.25*(x1+x2+x3+x4);
  if (m_StretchingFactor > 1) {
    vec3_t n0 = 0.25*(m_NodeNormal[id_cnode1] + m_NodeNormal[id_cnode2] + m_NodeNormal[id_cnode3] + m_NodeNormal[id_cnode4]);
    vec3_t n = m_NodeNormal[id_node];
    vec3_t v = x - x0;
    double sf = max(1.0, 1.0 + (m_StretchingFactor - 1.0)*pow(n0*n, 5.0));
    //x = x0 + (1.0/sf)*v;
