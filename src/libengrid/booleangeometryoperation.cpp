...
void BooleanGeometryOperation::fillGap()
{
    ...
    if (isSurface(id_cell, m_Grid)) {
      ...
      for (int i = 0; i < m_Part.c2cGSize(id_cell); ++i) {
        if (m_Part.c2cGG(id_cell, i) == -1) {
          ...
          E.id1 = ids[i+1];
          E.id2 = ids[i];
          E.bc = cell_code->GetValue(id_cell);
          E.id_face = id_cell;
          edges.append(E);
        }
      }
  ...
  QList<tri_t> tris;
  QVector<bool> free_node(m_Grid->GetNumberOfPoints());
  while (edges.size() > 0) {
    edge_t tri_edge = edges.last();
    vec3_t n1;
    if (tri_edge.id_face != -1) {
      n1 = cellNormal(m_Grid, tri_edge.id_face);
    } else {
      EG_BUG;
    }
    n1.normalise();
    ...
    for (vtkIdType id_node = 0; id_node < m_Grid->GetNumberOfPoints(); ++id_node) {
      free_node[id_node] = false;
    }
    foreach (edge_t edge, edges) {
      free_node[edge.id1] = true;
      free_node[edge.id2] = true;
    }
    QList<vtkIdType> nodes;
    for (vtkIdType id_node = 0; id_node < m_Grid->GetNumberOfPoints(); ++id_node) {
      if (free_node[id_node]) {
    ...
    double d_min = 1e99;
    vec3_t x1, x2;
    m_Grid->GetPoint(tri_edge.id1, x1.data());
    m_Grid->GetPoint(tri_edge.id2, x2.data());
    vec3_t v = x2 - x1;
    foreach (vtkIdType id_node1, nodes) {
      vec3_t x3;
      m_Grid->GetPoint(id_node1, x3.data());
      vec3_t cc;
      double R;
      GeometryTools::computeCircumscribedCircle(x1, x2, x3, cc, R);
      vtkIdType id3 = id_node1;
      foreach (vtkIdType id_node2, nodes) {
        if (id_node1 != id_node2) {
          vec3_t x3;
          m_Grid->GetPoint(id_node2, x3.data());
          vec3_t n2 = triNormal(x1, x2, x3);
          n2.normalise();
          if (n1*n2 > 0) {
            if ((cc - x3).abs() < R) {
              id3 = -1;
    ...
    if (id_best != -1) {
      tri_t tri;
      tri.id1 = tri_edge.id1;
      tri.id2 = tri_edge.id2;
      tri.id3 = id_best;
      tri.bc = tri_edge.bc;
      tris.append(tri);
    } else {
  ...
  EG_VTKSP(vtkUnstructuredGrid, grid);
  allocateGrid(grid, m_Grid->GetNumberOfCells() + tris.size(), m_Grid->GetNumberOfPoints());
  makeCopyNoAlloc(m_Grid, grid);
  {
    EG_VTKDCC(vtkIntArray, bc, grid, "cell_code");
    vtkIdType pts[3];
    foreach (tri_t tri, tris) {
      pts[0] = tri.id1;
      pts[1] = tri.id2;
      pts[2] = tri.id3;
      vtkIdType id_new = grid->InsertNextCell(VTK_TRIANGLE, 3, pts);
      bc->SetValue(id_new, tri.bc);
    }
  }
...
void BooleanGeometryOperation::fillGap()
{
  writeGrid(m_Grid, "original");
  EG_VTKSP(vtkGeometryFilter, extract_surface);
  extract_surface->SetInput(m_Grid);
  EG_VTKSP(vtkFeatureEdges, find_boundary);
  find_boundary->FeatureEdgesOff();
  find_boundary->NonManifoldEdgesOff();
  find_boundary->ManifoldEdgesOff();
  find_boundary->BoundaryEdgesOn();
  find_boundary->SetInput(extract_surface->GetOutput());
  EG_VTKSP(vtkDelaunay3D, triangulate);
  triangulate->SetInput(find_boundary->GetOutput());
  triangulate->Update();
  writeGrid(triangulate->GetOutput(), "tets");
}
*/
