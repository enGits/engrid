...
vtkIdType TriSurfaceProjection::m_LastPindex = 0;

TriSurfaceProjection::TriSurfaceProjection()
{
...
void TriSurfaceProjection::searchNewTriangle(vec3_t xp, vtkIdType &id_tri, vec3_t &x_proj, vec3_t &r_proj, bool neigh_mode, bool &on_triangle)
{
...
void TriSurfaceProjection::updateBackgroundGridInfo()
{
...
vtkIdType TriSurfaceProjection::getProjTriangle(vtkIdType id_node)
{
...
void TriSurfaceProjection::setProjTriangle(vtkIdType id_node, vtkIdType proj_triangle)
{
  EG_VTKDCN(vtkLongArray_t, pi, m_FGrid, "node_pindex");
  vtkIdType pindex = pi->GetValue(id_node);
  if (pindex < 0) {
...
vec3_t TriSurfaceProjection::project(vec3_t xp, vtkIdType id_node,  bool correct_curvature, vec3_t, bool, bool)
{
  ...
  if (proj_triangle == -1) {
    searchNewTriangle(xp, proj_triangle, x_proj, r_proj, false, on_triangle);
    if (id_node != -1) {
  ...
  if (!intersects || (d > m_CritDistance*T.smallestLength())) {
    searchNewTriangle(xp, proj_triangle, x_proj, r_proj, true, on_triangle);
    if (!on_triangle) {
      searchNewTriangle(xp, proj_triangle, x_proj, r_proj, false, on_triangle);
    }
...
void TriSurfaceProjection::computeSurfaceCurvature()
{
...
double TriSurfaceProjection::getRadius(vtkIdType id_node)
{
  vec3_t x;
