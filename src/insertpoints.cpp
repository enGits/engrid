...
int InsertPoints::insertPoints()
{
  ...
  foreach (edge_t E, edges) {
    if (E.S.id_cell.size() == 2) {
      if (E.S.type_cell[1] == VTK_TRIANGLE) {
  ...
  for (int i = 0; i < cells.size(); i++) {
    if (marked_cells[i] == 1) {
      ...
      copyNodeData(grid_tmp, S.p1, grid_tmp, id_new_node); ///@@@ maybe trouble
      ...
      if (S.id_cell.size() == 2) {
        if (S.type_cell[1] == VTK_TRIANGLE) { //2 triangles
          //four new triangles
          ...
          pts_triangle[2][0] = S.id_node[0];
          pts_triangle[2][1] = id_new_node;
          pts_triangle[2][2] = S.p2;

          pts_triangle[3][0] = id_new_node;
          pts_triangle[3][1] = S.id_node[1];
          pts_triangle[3][2] = S.p2;
          ...
          id_new_cell = grid_tmp->InsertNextCell(VTK_TRIANGLE, 3, pts_triangle[2]);
          copyCellData(grid_tmp, S.id_cell[1], grid_tmp, id_new_cell);

          id_new_cell = grid_tmp->InsertNextCell(VTK_TRIANGLE, 3, pts_triangle[3]);
          copyCellData(grid_tmp, S.id_cell[0], grid_tmp, id_new_cell);
        }
      } else if (S.id_cell.size() == 1) { //1 triangle
        //two new triangles
        ...
        vtkIdType id_new_cell;
        id_new_cell = grid_tmp->InsertNextCell(VTK_TRIANGLE, 3, pts_triangle[1]);
        copyCellData(grid_tmp, S.id_cell[0], grid_tmp, id_new_cell);
      } else {
